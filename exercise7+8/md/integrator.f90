MODULE integrator
    ...
    SUBROUTINE euler2 (x0,v0,dt,m,x,v)
        ...
        v = v0 + (f(x0)/m) * dt + (tdf(x0) * v0)/m * dt**2.0
    ...
    SUBROUTINE euler3 (x0,v0,dt,m,x,v)
        ...
        x = x0 + v0*dt + 0.5 * (f(x0)/m) * dt**2.0 + (tdf(x0) * v0)/(m*6.0) * dt**3.0
        v = v0 + (f(x0)/m) * dt + 0.5 * (tdf(x0) * v0)/m * dt**2 + &
        ((tdf2(x0) * v0**2)/m + (tdf(x0) * (f(x0)/m))/(m*6.0)) * dt**3.0
